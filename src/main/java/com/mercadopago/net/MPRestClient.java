...
public class MPRestClient {
  ...
  @Deprecated
  public MPApiResponse executeGenericRequest(
      HttpMethod httpMethod,
      String uri,
      PayloadType payloadType,
      JsonObject payload,
    ...
    try {
      ...
      if (!uri.contains("/oauth/token"))
        colHeaders.add(
            new BasicHeader("Authorization", String.format("Bearer " + SDK.getAccessToken())));
   ...
   * @param connectionTimeout int with the connection timeout for the api request expressed in
   *     milliseconds
   ...
   */
  public MPApiResponse executeRequest(
      HttpMethod httpMethod, String uri, PayloadType payloadType, JsonObject payload)
      throws MPRestException {
  ...
  public MPApiResponse executeRequest(
    ...
    try {
      ...
      HttpRequestBase request =
          createHttpRequest(httpMethod, uri, payloadType, payload, requestOptions);
      ...
      try {
        ...
        response =
            new BasicHttpResponse(new BasicStatusLine(request.getProtocolVersion(), 400, null));
      } catch (SSLPeerUnverifiedException e) {
        response =
            new BasicHttpResponse(new BasicStatusLine(request.getProtocolVersion(), 403, null));
      } catch (IOException e) {
        response =
            new BasicHttpResponse(new BasicStatusLine(request.getProtocolVersion(), 404, null));
      }
      long endMillis = System.currentTimeMillis();

      Stats stats =
          new Stats(context, request, response, startMillis, endMillis, startRequestMillis);
      stats.start();
  ...
  private HttpRequestBase createHttpRequest(
    ...
    Map<String, String> headers = new HashMap<String, String>();
    headers.put(
        HTTP.USER_AGENT, String.format("MercadoPago Java SDK/%s", MercadoPago.SDK.getVersion()));
    ...
    if (payloadType == PayloadType.JSON) {
      ...
      request.addHeader(
          new BasicHeader(HTTP.CONTENT_TYPE, ContentType.APPLICATION_FORM_URLENCODED.toString()));
    }
  ...
  private HttpClient createHttpClient() {
    ...
    DefaultHttpRequestRetryHandler retryHandler =
        new DefaultHttpRequestRetryHandler(MercadoPago.SDK.getRetries(), false);
   ...
   */
  private HttpEntity normalizePayload(PayloadType payloadType, JsonObject payload)
      throws MPRestException {
    ...
    if (payload != null && payload.size() != 0) {
      if (payloadType == PayloadType.JSON) {
        ...
        Map<String, Object> map =
            new Gson()
                .fromJson(payload.toString(), new TypeToken<Map<String, Object>>() {}.getType());
   ...
   * @throws MPRestException
   */
  private HttpRequestBase getRequestMethod(HttpMethod httpMethod, String uri, HttpEntity entity)
      throws MPRestException {
    if (httpMethod == null) {
      throw new MPRestException("HttpMethod must be \"GET\", \"POST\", \"PUT\" or \"DELETE\".");
    }
    if (StringUtils.isEmpty(uri)) throw new MPRestException("Uri can not be an empty String.");
