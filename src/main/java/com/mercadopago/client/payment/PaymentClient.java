...
public class PaymentClient extends MercadoPagoClient {
  ...
  /**
   * Returns a lazy iterable that auto-fetches all pages of payments matching the search criteria.
   *
   * @param request search filters; {@code limit} and {@code offset} control page size/start
   * @return an {@link Iterable} of {@link Payment} backed by {@link MPAutoPagingIterable}
   * @throws MPException    if an error occurs while fetching
   * @throws MPApiException if the API returns a non-success status code
   */
  public Iterable<Payment> searchAll(MPSearchRequest request) throws MPException, MPApiException {
   ...
   * @throws MPApiException if the API returns a non-success status code
   */
  public Iterable<Payment> searchAll(MPSearchRequest request, MPRequestOptions requestOptions)
      throws MPException, MPApiException {
    return new MPAutoPagingIterable<>(
        (req, opts) -> this.search(req, opts), request, requestOptions, Payment.class);
  }
